...
RSTC.prototype.handleArguments = function(env) {
	if (this.minimist.verbose) {
		...
		console.log('RSTC OPTIONS: ' + util.inspect(this.minimist, false, null));
	...
	debug('env.modulePath: %s', env.modulePath);
	/*
		Takes care that rstc-cli is locally installed.
		Might as well get rid of Liftoff and use 'resolve' like in ember-cli
		(to resolve to a local installation or fall back on global).
	/*

	/*
	if (!env.modulePath) {
		console.log('Local ' + this.liftoff.moduleName + ' module not found in: ' + chalk.cyan(env.cwd));
		exit(1);
	}
	*/

	// env.configPath - 'Hackerfile' is just an empty file (at least atm).
	/*
	if (env.configPath) {
