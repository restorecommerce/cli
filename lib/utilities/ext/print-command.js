...
module.exports = function(options) {
  ...
  var shouldDescriptionBeGrey = options.shouldDescriptionBeGrey;

  var output = '';

  var isSuperCommand = Object.keys(this.availableCommands).length;

  if(isSuperCommand && this.availableOptions.length) {
  ...
  // <options...>
  if (!isSuperCommand && options.length) {
    output += ' ' + chalk.cyan('<options>');
  }
  ...
  if (description) {
    ...
    output += EOL + margin + '  ' + description;
  }

  // aliases: a b c
  if (this.aliases && this.aliases.length) {
    output += EOL + margin + '  ' + chalk.grey('aliases: ' + this.aliases.filter(function(a) { return a; }).join(', '));
  }

  // --available-option (Required) (Default: value)
  // ...
  options.forEach(function(option) {
    output += EOL + margin + '  ' + chalk.cyan('--' + option.name);
    ...
    if (option.aliases && option.aliases.length) {
      output += EOL + margin + '    ' + chalk.grey('aliases: ' + option.aliases.map(function(a) {
        if (typeof a === 'string') {
