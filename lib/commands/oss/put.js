 ...
 */
	...
	run: function(commandOptions, rawArgs) {
		...
		const pathToJson = rawArgs.shift();
		if(!pathToJson) {
			console.log(this.printBasicHelp(commandOptions));
			const message = 'A path to a JSON file must be provided.';
			return new Promise.reject(new SilentError(message));
		}
		...
		try {
			...
			return new Promise.reject(new SilentError(e));
		}
			...
			if (commandOptions[camelized] !== option.default)
				if (json[option.name]) {
					console.log(chalk.red('Overrided JSON file\'s (' + pathToJson + ') "' + option.name +
					 						'" (' + json[option.name] + ') with: "' + option.default+'".'));
				} else commandOptions[camelized] = json[option.name];
		}.bind(this));
