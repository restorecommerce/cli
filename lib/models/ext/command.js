...
Command.prototype.validateAndRun = function(args) {
  ...
  // Command has sub-commands, so the first non-option argument is the next command.
  if (Object.keys(this.availableCommands).length) {
  ...
  // Command doesn't have sub-commands, so all of the non-option arguments are parameters.
...
Command.prototype.parseTill = function(_args) {
  const hasSubCommands = Object.keys(this.availableCommands).length;
  debug('hasSubCommands: ', hasSubCommands);
  ...
  args.some(function(arg, i) {
    if (!skip) {
      if (arg[0] !== '-') {
        if (hasSubCommands) return true; // No options specified for current command.
      } else {
