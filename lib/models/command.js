...
const Command = CoreObject.extend({
  init() {
...
Command.prototype.validateAndRun = function (args, previousOptions) {
  ...
  if (Object.keys(this.availableCommands).length) {
    debug('"' + this.name + ' has sub-commands. Trying to merge options..');
    try {
      debug('Merging "' + this.name + '" options: ', this.availableOptions);

      let arg = args.find(arg => {
      ...
      if(arg) {
        nextCommand = stringUtils.classify(arg);
      }
      ...
      nextCommand = new this.availableCommands[nextCommand];
      debug('.. with "' + arg + '" options:', nextCommand.availableOptions);

      this.registerOptions({
        availableOptions: nextCommand.availableOptions
      });

      debug('Merged options: ', this.availableOptions);
    } catch (e) {
      debug(e);
      debug('Continuing without merged options.');
    }
  }
  ...
  */
  ...
  defaults(previousOptions, commandOptions.options);
  return this.run(previousOptions, commandOptions.args);
};
...
Command.prototype.printBasicHelp = function (showSubCommands) {
  var output = this.getHelp({
    fullDescriptor: true
  });

  if (showSubCommands)
    output += this.getAvailableCommands();
...
Command.prototype.printDetailedHelp = function () {
  this.printBasicHelp(true);
};

Command.prototype.getAvailableCommands = function () {
  var output = '';
  if (Object.keys(this.availableCommands).length) {
    output += ('\n Where ' + chalk.yellow('<command>') + ' is one of:\n\n');

    Object.keys(this.availableCommands).forEach(function (commandName) {
      const command = new this.availableCommands[commandName];
...
Command.prototype.validateAlias = function (option, alias) {
  ...
  if (!this.optionsAliases[key]) {
    ...
    if (value[0] !== this.optionsAliases[key][0]) {
      throw new Error('The "' + key + '" alias is already in use by the "' + this.optionsAliases[key][0] +
...
Command.prototype.parseArgs = function (commandArgs) {
  ...
  var validateParsed = function (key) {
    if (!commandOptions.hasOwnProperty(key) && key !== 'argv') {
      ...
      console.log(chalk.yellow(message)); // Warning
