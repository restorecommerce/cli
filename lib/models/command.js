...
Command.prototype.parseTill = function(_args) {
  ...
  function invalidOptionError(invalidOption, name) {
    return new SilentError('The option "--' + invalidOption + '"" is not registered with the ' +
                            name + ' command. ' + 'Run "rstc ' + name +
                            ' --help" for a list of supported options.');
  }

  function validOptionAlias(arg, nextArgument) {
    var validAlias = this.optionsAliases.hasOwnProperty(arg);
    if(validAlias) {
      var option;

      this.availableOptions.some(function(availableOption) {
        if(availableOption.aliases.indexOf(arg) !== -1) {
          option = availableOption;
          return true;
        }
      });

      skip = skipNextArgument(option.type, nextArgument);
      return true;
    }
  ...
  function validOptionName(arg, nextArgument) {
    return this.availableOptions.some(function(option) {
      if (arg === option.name) {
        skip = skipNextArgument(option.type, nextArgument);
        return true;
      }
  ...
  args.some(function(arg, i) {
    if (!skip) {
      if (arg[0] !== '-') return true; // Exit, no options specified for current command.
      while (arg[0] === '-') arg = arg.slice(1, arg.length);

      var nextArgument = args[i + 1];
      var validOption = validOptionAlias.bind(this)(arg, nextArgument);
      if(!validOption) validOption = validOptionName.bind(this)(arg, nextArgument);

      if (!validOption) throw invalidOptionError(arg, this.name);

      parseTill = i;
      if (skip) // Skip is set either inside 'validOptionAlias' or 'validOptionName'.
        parseTill++;
...
Command.prototype.getBasicHelp = function(options) {
  var output = '';
  if (this.isRoot) {
    output = 'Usage: ' + this.name;
  } else if (options) {
    if(options.margin) {
      output += options.margin;
      options.initialMargin = options.initialMargin ? options.initalMargin + options.margin : options.margin;// + options.margin;
    }

    if (options.useFullDescriptor) {
      if (this.superCommand) {
        const commandDescriptor = this.getFullCommandDescriptor();
        output += 'rstc ' + commandDescriptor;
      } else {
...
Command.prototype.getFullCommandDescriptor = function() {
  return this._getSuperCommandChain(function(cmd, arr) {
    const hasOptions = Object.keys(cmd.availableCommands).length;
...
Command.prototype.printBasicHelp = function(options) {
  var output = this.getBasicHelp({
    useFullDescriptor: true
  });

  var _options = Object.assign({}, options);
  _options.margin = ' '; // Add margin to sub-command list.

  output += this.getAvailableCommands(_options);
  console.log(output);
};

Command.prototype.lookupAvailableCommands = function() {
  const commandChain = this.getSuperCommandChain('/')
  const commandPath = '../commands/' + commandChain + '/*.js'; // path.resolve(__dirname, '../commands/') + '/' + commandChain + '/*.js;';
...
Command.prototype.printAvailableCommands = function(options) {
  const output = this.getAvailableCommands(options);
  console.log(output);
};
...
Command.prototype.getAvailableCommands = function(options) {
  var output = '';
  if (Object.keys(this.availableCommands).length) {
    output += ('\n'+ (options && options.margin ? options.margin : '') +'Where ' + chalk.yellow('<command>') + ' is one of:\n\n');
