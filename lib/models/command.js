...
let Command = CoreObject.extend({
  ...
  [
    ...
    { name: 'help',    type: Boolean, default: false, aliases: ['h'] },
    { name: 'entry',   type: Boolean, default: false },
  ],*/
  anonymousOptions: []
	/*
    this.isWithinProject = this.project.isrstcCLIProject();
...
Command.prototype.validateAndRun = function(args) {
  ...
  // TO DO: Delete ^^^ and make it work like it's commented out below.
  ...
  if (this.works === 'insideProject' && !this.isWithinProject) {
    return Promise.reject(new SilentError(
      'You have to be inside an rstc-cli project in order to use ' +
  ...
  if (this.works === 'outsideProject' && this.isWithinProject) {
    return Promise.reject(new SilentError(
      ...
      ' command inside an rstc-cli project.'
    ));
  }
